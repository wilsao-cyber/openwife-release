"""
MCP Client for Desktop Control (cross-platform)
讓 AI 老婆可以控制桌面操作
"""
...
class MCPDesktopControl:
    """MCP Client for desktop-control-mcp (cross-platform)"""

    def __init__(self, mcp_server_path: str = None):
        self.mcp_server_path = mcp_server_path or ("windows-desktop-control" if IS_WINDOWS else "ubuntu-desktop-control")
    ...
    async def start(self):
        ...
        logger.info(f"Starting {self.mcp_server_path} MCP server...")
        try:
            ...
            logger.warning(
                ...
                "Desktop control is disabled. Install the appropriate desktop-control-mcp for your OS."
            )
            self._process = None
            self._initialized = False
